...
def trades_loss_weighted(model,
                x_natural,
                y,
                optimizer,
                weight,
    ...
    one_hot_label = F.one_hot(y, 10).float()
    loss_natural = weighted_cross_entropy(logits, one_hot_label, weight)
    loss_robust = (1.0 / batch_size) * criterion_kl(F.log_softmax(model(x_adv), dim=1),
                                                    F.softmax(model(x_natural), dim=1))
    loss = loss_natural + beta * loss_robust
    return loss
...
def my_trades_loss(model, optimizer, images, labels, adversary_algo):
    criterion_kl = nn.KLDivLoss(size_average=False)
    # perform image perturbation
    with ctx_noparamgrad_and_eval(model):
        images_adv = adversary_algo.perturb(images, labels)
    optimizer.zero_grad()
    # calculate robust loss
    outputs = model(images)
    loss_natural = F.cross_entropy(outputs, labels)
    loss_robust = (1.0 / len(images)) * criterion_kl(F.log_softmax(model(images_adv), dim=1),
                                                F.softmax(model(images), dim=1))
    loss = loss_natural + 1.0 * loss_robust
    return loss